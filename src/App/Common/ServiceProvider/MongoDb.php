 ...
 */
class MongoDb implements ServiceProviderInterface
{
    const VERSION = '0.0.0';
     ...
     * @return string
     */
    private function assembleUri($host, $port, $db = null, $username = null, $password = null, $replica = false)
    {
        if (!empty($db)) {
            $db = '/' . $db;
        }
        ...
        $uri = sprintf('mongodb://%s%s:%s%s', $cred, $host, $port, $db . '');
        if ($replica) {
            $uri = sprintf('mongodb+srv://%s%s', $cred, $host);
        }
        return $uri;
    }
}
